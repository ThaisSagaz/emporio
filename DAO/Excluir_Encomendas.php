...
if (!isset($_SESSION['UsuarioID'])) {
  ...
  if ( !empty($_POST)) {
    ...
    $sql = "DELETE FROM produtos_has_encomedas WHERE Encomedas_cod_Encomenda = ?;
        DELETE FROM encomendas WHERE cod_Encomenda = ?;       ";
    $q = $pdo->prepare($sql);
...
<html> <?php  if (!isset($_SESSION)) session_start();  // Verifica se não há a variável da sessão que identifica o usuário if (!isset($_SESSION['UsuarioID'])) {     // Destrói a sessão por segurança     session_destroy();     // Redireciona o visitante de volta pro login     header("Location: inicio.php"); exit; } ?>
  ...
  <head>
    ...
    <title>Excluir encomendas</title>
        ...
        <div class="box" id="form">
              ...
              <div class="box box-info" id="form">
                <div class="box-header with-border">
                  <h3 class="box-title">Excluir Encomenda</h3>
                </div><!-- /.box-header -->
                <!-- form start -->
               <form class="form-horizontal" id="form" action="Excluir_Encomendas.php" method="post">

                  <div class="box-body">

                  <input type="hidden" name="id" value="<?php echo $id;?>"/>
              <p class="alert alert-error">Tem certeza que quer excluir esta encomenda?</p>
                  ...
                  <div class="box-footer">
                    ...
                    <a class="btn btn-default" href="../encomendas.php">Não</a>
                  </div><!-- /.box-footer -->
                </form>
                </div>
          </div><!-- /.box -->
